...
function createCategoryFilters() {
    const categories = ['all', 'Computer Science', 'Mathematics', 'Economics', 'Physics', 'Chemistry'];
...
async function downloadMaterial(id) {
    ...
    try {
        ...
        fetch(`${API_BASE}/materials/${id}/download`, {
            method: 'POST',
            credentials: 'include'
        }).catch(() => {}); // Don't block download if this fails
        ...
        // API_BASE is like "http://localhost:5000/api"
        // downloadURL is like "/api/download/filename.pdf"
        const serverUrl = API_BASE.replace('/api', '');
        const fullUrl = serverUrl + material.downloadURL;

        console.log('Downloading from:', fullUrl);

        // Open in new tab - most reliable cross-origin method
...
function previewMaterial(id) {
    const material = materials.find((m) => m.id === id);
    if (material && material.downloadURL) {
        const baseUrl = API_BASE.replace('/api', ''); window.open(baseUrl + material.downloadURL, '_blank');
    }
}
...
async function handleUpload(e) {
    ...
    if (!progressBar) {
        ...
        progressBar.innerHTML = '<div class="bg-[#4F46E5] h-2.5 rounded-full transition-all duration-300" style="width: 0%"></div>';
...
document.addEventListener('keydown', function (e) {
    if (e.key === '/' && document.getElementById('materials').classList.contains('hidden') === false) {
        e.preventDefault();
...
async function initializeApp() {
    ...
    setTimeout(() => {
        if (!document.getElementById('home').classList.contains('hidden')) {
            showToast(currentUser ? `Welcome back, ${currentUser.first_name}! 👋` : 'Welcome to StudyHub 👋');
        }
    }, 2500);
}
